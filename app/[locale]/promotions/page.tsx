...
const route: ShopRouteTarget = { pathname: "/promotions" };
...
export default async function PromotionsPage({
  ...
  // F03 : liste prédéfinie (badge promo), pas de filtres/tri.
  const [{ products, hasMore, nextCount }, categories] = await Promise.all([
    getShopResults({ ...shopParams, badge: "promo" }),
    getCategories(),
  ]);
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-10 lg:px-8">
      <div className="mb-8">
        <h1 className="font-serif text-4xl font-bold">{t("promotionsTitle")}</h1>
        <p className="mt-2 text-sm text-os-muted">{t("promotionsSubtitle")}</p>
      </div>
      <ProductGrid
        products={products}
